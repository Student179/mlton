...
structure ContextInfo =
    struct
        ...
        fun newHash (context, var) = Word.xorb((Context.hash context), (Var.hash var))
      ...
      val _ = 
        ...
        | Control.CFA.SimplyTypedCFA =>
          let
            ...
            val {hom = valueFromType, destroy} =
               Sxml.Type.makeMonoHom 
              ({con = fn (t, tycon, avs) =>
                  if Tycon.equals (tycon, Tycon.arrow)
                      then let
                              val av = Value.fromType t
                           in
                              av
                           end
                  else if Tycon.equals (tycon, Tycon.array)
                      then Value.arrayM (Vector.sub(avs, 0))
                  else if Tycon.equals (tycon, Tycon.vector)
                      then Value.vectorM (Vector.sub(avs, 0))
                  else if Tycon.equals (tycon, Tycon.weak)
                      then Value.weakM (Vector.sub(avs, 0))
                  else if Tycon.equals (tycon, Tycon.reff)
                      then Value.refM (Vector.sub(avs, 0))
                  else if Tycon.equals (tycon, Tycon.tuple)
                      then Value.tuple (avs)
                  else
                      Value.fromType t
               })
            (* The new foreach that does the simple type analysis *)
            val _ =
               Vector.foreach
               (datatypes, fn {cons, ...} =>
                Vector.foreach
            ...
            val _ =
               let
                  ...
                  and loopBind arg =
                     ...
                     (fn {var, ty, exp} =>
                      let
                        val () = newVar (var, valueFromType ty)
                        datatype z = datatype PrimExp.t
                     in
                        case exp of
                           App _ => ()
                         | Case {cases, default, ...} =>
                              let
                                 fun handlePat (Pat.T {arg, ...}) =
                                    case arg of
                                       NONE => ()
                                     | SOME (x, ty) => newVar (x, valueFromType ty)
                         ...
                         | Handle {try, catch = (x, t), handler} =>
                              let
                                 val () = loopExp try
                                 val () = newVar (x, valueFromType t)
                                 val () = loopExp handler
                              in ()
                              end
                         | Lambda l => loopLambda (l, var, ty)
                         | PrimApp _ => ()
                         | Profile _ => ()
                         | Raise _ => ()
                         | Select _ => ()
                  ...
                  and loopLambda (lambda: Lambda.t, x: Var.t, ty: Type.t): unit =
                     let
                        val _ = List.push (allLambdas, lambda)
                        val () = Value.unify(valueFromType ty, 
                                            Value.lambda (lambda, Context.new [], ty))
                        val _ =
                           setLambdaInfo
                           (lambda,
                            LambdaInfo.T {con = ref Con.bogus,
          ...
          let
            ...
            val _ =
               let
                  ...
                  fun setVar (c: Context.t, x: Var.t, v: Value.t): unit = 
                    let
                  ...
                  fun lookUpVar (c:Context.t, x: Var.t, i: Int.t) : Value.t =
                      let
                        val info = ContextInfo.new(c, x)
                        val valueOpt = ContextValueMap.lookUp(contextValueMap, info)
                        val error = concat["HashMap LookUp failed at ", Int.toString(i)]
                      in
                        case valueOpt of
                          SOME v => v
                        | NONE => let
                  ...
                  fun lookUp (c: Context.t, i: Int.t) (x: SvarExp.t) : Value.t =
                      lookUpVar(c, SvarExp.var x, i)
                  fun lookUpXs (c, i) xs = Vector.map (xs, (lookUp (c, i)))
                  fun mapFrees(c: Context.t, lambda: Lambda.t, c': Context.t): unit =
                    let
                      val LambdaInfo.T {frees, ...} = lambdaInfo lambda
                      fun lookSetVar (x: Var.t): unit = setVar(c', x, (lookUpVar(c, x, 0)))
                    in
                      Vector.foreach(!frees, lookSetVar)
                    end
                  (* ---------------------------------- *)
                  (*    Main CFA Program Walkthrough    *)
                  (* ---------------------------------- *)
                  ...
                  and loopDec (c: Context.t) (d: Dec.t): unit =
                     ...
                     in
                        case d of
                           Fun {decs, ...} =>
                              (Vector.foreach (decs, fn {var, lambda, ty, ...} =>
                                              setVar(c, var, Value.fromType ty)))
                  ...
                  and loopBind (c: Context.t, arg): unit =
                     ...
                     (fn {var, ty, exp} =>
                     let
                        fun set v = setVar(c, var, v)
                        fun newV (x, v) = setVar(c, x, v)
                        fun new () =
                         let val v = Value.fromType ty
                         in set v; v
                         end
                     ...
                     in
                        case exp of
                           App {func, arg} =>
                            let 
                              val argV = lookUp (c, 2) arg
                              val result = new ()
                              val c' = Context.new [var]
                            in Value.addHandler
                              (lookUp (c, 3) func, fn l =>
                              let
                                 val (lambda, cx) = Value.Lambda.dest l
                                 val results  = loopLambda (cx, lambda, c')
                                 val {arg = formal, body, ...} = Lambda.dest lambda
                              in
                                  Value.coerce {from = argV,
                                                to = lookUpVar(c', formal, 4)}
                         ...
                         | ConApp {con, arg, ...} =>
                              (case (arg,    conArg con) of
                                  (NONE,   NONE)       => ()
                                | (SOME x, SOME v)     =>
                                     Value.coerce {from = lookUp (c, 5) x, to = v}
                                | _ => Error.bug "ClosureConvert.loopBind: ConApp"
                               ; new' ())
                         | Const _ => new' ()
                         | Handle {try, catch = (x, t), handler} =>
                         ...
                         | Lambda l => set (Value.lambda (l, c, ty))
                         | PrimApp {prim, args, ...} =>
                              set (Value.primApply {prim = prim,
                                                    args = lookUpXs (c, 6) args,
                                                    resultTy = ty})
                         | Profile _ => new' ()
                         | Raise _ => new' ()
                         | Select {tuple, offset} =>
                              set (Value.select (lookUp (c, 7) tuple, offset))
                         | Tuple xs =>
                            if Value.typeIsFirstOrder ty
                               then new' ()
                            else set (Value.tuple (Vector.map (xs, lookUp (c, 8))))
                         | Var x => set (lookUp (c, 9) x)
                     end) arg
